...
savename_EML = sprintf("%s_EML_conf", datname);
modelname_base = sprintf("model_%s_NL=3_nEpoch=2000_lossFunc=MSE_untied=T_vgg=F_unet_nfilts=0_act=shrink", savename_EML);
modelname_fc = sprintf("model_FC_conf");
modelname_automap = sprintf("model_automap_conf");
...
[reconIms_admm, reconTime_admm] = reconADMM_exp(datpath, resultpath, savename_EML, testInds, admmOpts,...
    "J_L_conf", "diff_L_conf");
[reconIms_fista, reconTime_fista] = reconFISTA_exp(datpath, resultpath, savename_EML, testInds, fistaOpts,...
    "J_L_conf", "diff_L_conf");

run_runtime_cmd_fc = sprintf("python3 ../fig12_recon_exp/support_scripts/unrolled_DOT_exp_test.py %s", modelname_fc);
...
run_runtime_cmd_automap = sprintf("python3 ../fig12_recon_exp/support_scripts/unrolled_DOT_exp_test.py %s", modelname_automap);
...
run_runtime_cmd_base = sprintf("python3 ../fig12_recon_exp/support_scripts/unrolled_DOT_exp_test.py %s", modelname_base);
...
runtime_arr_ms = 1000*[reconTime_admm, reconTime_fista, recon_fc.reconTime_unrolled, recon_automap.reconTime_unrolled, recon_unrolled_base.reconTime_unrolled];
...
scatter(mse_arr(1:(end-1)), runtime_arr_ms(1:(end-1)), 100, 'filled');
hold on;
scatter(mse_arr(end), runtime_arr_ms(end), 100, 'filled');
...
ax_ticks = [3.5e-2, 6e-2, 15e-2, 30e-2];
