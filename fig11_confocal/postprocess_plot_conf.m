...
addpath(genpath("../fig12_recon_exp"));
...
admm_path = sprintf("%s/exp/recon_result_admm_widefield_allTrainingDat_30-Sep-2021_EML_conf", resultpath);
fista_path = sprintf("%s/exp/recon_result_fista_ToF_widefield_allTrainingDat_30-Sep-2021_EML_conf", resultpath);
fc_path = sprintf("%s/exp/unrolled_dot_exp_eml_test_model_FC_conf", resultpath);
automap_path = sprintf("%s/exp/unrolled_dot_exp_eml_test_model_automap_conf", resultpath);
unrolled_path = sprintf("%s/exp/unrolled_dot_exp_eml_test_model_allTrainingDat_30-Sep-2021_EML_conf_NL=3_nEpoch=2000_lossFunc=MSE_untied=T_vgg=F_unet_nfilts=0_act=shrink", resultpath);

admm_struct = load(admm_path);
fista_struct = load(fista_path);
recon_fc = load(fc_path);
recon_automap = load(automap_path);
recon_unrolled_base = load(unrolled_path);
...
f_tof = figure('Position', [40 440 880 440]);
...
savename = "recon_out";
fullsavepath = fullfile(resultpath, 'confocal', savename);
...
unrolled_ssim = mean(ssim(reconIms_unrolled_base, truthIms_test, 'DataFormat', 'SSB'));
...
reconTime_unrolled_ms = 1000*recon_unrolled_base.reconTime_unrolled;
...
scatter(reconTime_unrolled_ms, unrolled_ssim, star_sz, unrolled_sym, 'filled', 'MarkerEdgeColor', 'y', 'LineWidth', 2);
ylabel("SSIM");
yyaxis right;
hold on;
...
scatter(reconTime_unrolled_ms, avg_mse_unrolled_base, star_sz, unrolled_sym, 'filled', 'MarkerEdgeColor', 'y', 'LineWidth', 2);
set(gca, 'YScale', 'log');
set(gca, 'XScale', 'log');
xlabel("Runtime (ms)");
ylabel("MSE");
...
export_fig(f, sprintf("%s/confocal/recon_result", resultpath), '-png', '-m3', '-transparent');

%% 5. Export metrics

label_arr = ["ADMM", "FISTA", "FC", "Automap", "Unrolled-DOT (base)"];
mse_arr = [avg_mse_admm, avg_mse_fista, avg_mse_fc, avg_mse_automap, avg_mse_unrolled_base];
runtime_arr_ms = [reconTime_admm_ms, reconTime_fista_ms, reconTime_fc_ms, reconTime_automap_ms, reconTime_unrolled_ms];
ssim_arr = [admm_ssim, fista_ssim, fc_ssim, automap_ssim, unrolled_ssim];

savedir = fullfile(resultpath, 'confocal');
final_report_str = "";
...
fid = fopen(fullfile(resultpath, 'confocal', report_filename), 'w');
